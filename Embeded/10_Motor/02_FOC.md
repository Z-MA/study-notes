...
在底点对称轴对正弦波采样，采样值作为载波交点控制开关器件的通断。三角波的幅值就是 $𝑈_𝑇$ ，正弦波的幅值就是 $𝑈_𝑆$，将正弦波向上平移 1 个单位，使其在时间轴上方，根据相似三角形原理，可得如下关系式。其式基于双极性调整法，$U_T$为单位电压1V，
...
$$\delta = T_C(1+asin(\omega t_D)/2\\(其中调制比a=\frac{U_S}{U_T},\omega t_D=\frac{2k\pi}{N},k 为一个正弦波周期内的采样计数值,得到的𝛿就是开关时长。)$$
