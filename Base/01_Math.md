...
常数函数：$(c)' = 0$（c为常数）
幂函数：$(x^n)' = n·x^{n-1}$
指数函数：$(a^x)' = a^x · ln a （a > 0, a ≠ 1）$
(e^x)' = e^x （自然指数函数）

对数函数：$(log_a x)' = 1/(x·ln a) （a > 0, a ≠ 1）$
- `(ln x)' = 1/x` （自然对数）
...
除法法则：$[f(x)/g(x)]' = [f'(x)g(x) - f(x)g'(x)] / [g(x)]²$（上导下不导 - 上不导下导）除以分母的平方）
- 例：`(sin x / x)' = (cos x·x - sin x·1)/x²`

链式法则：${f[g(x)]}' = f'[g(x)]·g'(x)$
- 设 `u = g(x)`，则 `dy/dx = dy/du · du/dx`
- 例：`sin(2x)`，令 `u=2x`，导数为 `cos u · 2 = 2cos(2x)`
...
### 复合函数求导
1. `f(x) = e^(sin x)` → `f'(x) = e^(sin x)·cos x`
2. `f(x) = ln(x² + 1)` → `f'(x) = 2x/(x² + 1)`
3. `f(x) = (2x+1)¹⁰` → `f'(x) = 20(2x+1)⁹`

### 乘积和商求导
- **乘积**：`f(x) = x² ln x` → `f'(x) = 2x ln x + x`
- **商**：`f(x) = e^x / x` → `f'(x) = e^x(x-1)/x²`
