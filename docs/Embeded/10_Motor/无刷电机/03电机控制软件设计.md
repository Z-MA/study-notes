...
>$$\begin{cases}
 P_{等功率变换}=V_q\times I_q+V_d\times I_d\\
 P_{等幅变换}=\frac{3}{2}\times(V_q\times I_q+V_d\times I_d)
\end{cases}$$
>2. 实际FOC设计中一般采用等幅变换，因为在svpwm设计中假如采用等功率变换，得到的矢量$U_t$为相电压峰值的1.5倍，将会超出空间矢量的圆圈，产生过调制。具体分析可参考以下链接：
>>1. [知乎.胡说.SVPWM调制中的6个非零基础电压矢量的幅值到底是Udc还是2/3Udc ?[BE/OL].(2023.1.17)[2024.1.31]](https://zhuanlan.zhihu.com/p/56529497)
>>2. [知乎.Panda.关于矢量控制clarke变化2/3和sqrt（2/3）的理解[BE/OL].(2020.3.13)[2024.1.31]](https://zhuanlan.zhihu.com/p/112704657)
>>3. [CSDN.w_乐天.Clark变换及比例系数2/3推导过程[BE/OL].(2019.5.7)[2024.1.31]](https://blog.csdn.net/daidi1989/article/details/89926324)
...
$$
U(t)=U_{A}(t)+U_{B}(t) e^{j 2 \pi / 3}+U_{C}(t) e^{j 4 \pi / 3}=\frac{3}{2} U_{m} e^{j \theta} （公式1-10）
$$
...
\\ \left|U_{r e f}\right| \sin \theta=\frac{T_{6}}{T_{s}}\left|U_{6}\right| \sin \frac{\pi}{3} & \beta \text { 轴 }\end{cases} （公式1-12）
$$
...
>|扇区|落在此扇区的充要条件 |
>|  ----  |  ----  |
>|Ⅰ | $U_\alpha > 0,U_\beta > 0 且 U_\beta /U_\alpha < \sqrt{3} $ |
>|Ⅱ | $U_\alpha > 0, 且 U_\beta /U_\alpha > \sqrt{3}$|
>|Ⅲ |$U_\alpha < 0,U_\beta > 0 且 -U_\beta /U_\alpha < \sqrt{3} $|
>|Ⅳ |$U_\alpha < 0,U_\beta < 0 且 U_\beta /U_\alpha < \sqrt{3} $|
>|Ⅴ |$ U_\beta < 0, 且 -U_\beta /U_\alpha > \sqrt{3} $|
>|Ⅵ | $U_\alpha > 0,U_\beta < 0 且 -U_\beta /U_\alpha < \sqrt{3} $|
