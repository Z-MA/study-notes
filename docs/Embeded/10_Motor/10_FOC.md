...
- 如上图，当 $U_1 * T_\text{单位时间}$ 时，构成六边形的边界。
- 当 $U_1\&U_6$ 分别作用半个单位单位时间时，调制出的电压矢量是如图 $U_{Vr1}$，同理可得$U_{Vr2}$。
- 而 $U_{Vr1}$ 是合成电压矢量边界中的最小电压矢量，要实现幅值一致的旋转电压矢量，应该以六边形的内切圆作为最大输出电压矢量。
...
- 发波前确定转子所处机械角度，多对极时用机械角度*极对数；
- 对各相电流值进行采样，通过clark变换和park变换得到 $d$、$q$ 轴电流值；
- 计算电流值 和目标电流值计算出 $d$、$q$ 轴电流误差，通过PI调节器计算出 $d$、$q$ 轴电压指令值；
