...
- 总线上的电平有显性电平(Dominant)和隐性电平(Recessive)两种。
...
> 基本 ID 和标准格式的 ID 相同。禁止高 7 位都为隐性。  
> RTR：远程传输请求位，只在标准帧有，0表示数据帧、1表示远程帧。  
> SRR：占位符，只在扩展帧有，用作替换标准帧的 RTR 位，固定为1。在扩展帧中，RTR 远程传输请求位后移。  
> IDE：标识符扩展位，标准帧是 0，扩展帧是 1。  
> SRR 和 IDE 位保证了在标准帧与扩展帧共存时，标准帧优先级更高。  
> r0、r1：都是保留位，都是显性电平 0。
...
数据帧和远程帧有标准格式（11位标识符I D）和扩展格式（29位ID）
